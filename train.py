"""
Complete RL Pipeline with Phi-3.5-mini and HarmBench Data

Uses:
- Attacker: microsoft/Phi-3.5-mini-instruct (via UnifiedCategoryAttacker)
- Scorer: microsoft/Phi-3.5-mini-instruct (via RankBasedScorer)
- Target: microsoft/Phi-3.5-mini-instruct (via TargetModel wrapper)
...
def run_one_iteration_with_harmbench():
    """
    Run one complete iteration with Phi-3.5 and HarmBench data.
    Implements NxM evaluation and ranking aggregation.
    """
    
    print("="*80)
    print("RL PIPELINE: PHI-3.5 WITH HARMBENCH DATA (NxM EVALUATION)")
    print("="*80)
    ...
    LOAD_IN_8BIT = True  # Set to True to save memory
    
    # Initialize scorer
    print("\n1. Loading Phi-3.5-mini-instruct for scorer...")
    scorer = RankBasedScorer(
        model_name=MODEL_NAME,
        device='cuda',  # GPU 1  # GPU 1
        load_in_8bit=LOAD_IN_8BIT
    )
    
    # Initialize target model
    print("\n2. Loading Phi-3.5-mini-instruct for target...")
    target_model = TargetModel(
    ...
    # ------------------------------------------------------------------------
    # STEP 1: Generate N Attacker Prompts
    # ------------------------------------------------------------------------
    print("\n" + "="*80)
    print("STEP 1: Generate N Attacker Prompts")
    print("="*80)
    
    print("\n1. Loading attacker model...")
    attacker = UnifiedCategoryAttacker(
        model_name_or_path=MODEL_NAME,
        device='cuda',  # GPU 1  # GPU 0
        load_in_8bit=LOAD_IN_8BIT
    )
    ...
    # ------------------------------------------------------------------------
    # STEP 3: Aggregate Rankings Across Queries
    # ------------------------------------------------------------------------
    print("\n" + "="*80)
    ...
    print("\nHow this works:")
    print("  1. For each query, rank the N attacker prompts")
    ...
    rl_trainer = RankingToRLTrainer(
        model_name_or_path=MODEL_NAME,
        learning_rate=5e-6  # Lower LR for Phi-3.5
    )
    ...
    rl_trainer.train_from_rankings(
        ...
        epochs=2,
        batch_size=2,
        save_path="./phi35_trained_attacker"
    )
    
    print("\n✓ Training complete!")
    print("  Saved to: ./phi35_trained_attacker")
    ...
    attacker.load_finetuned_model("./phi35_trained_attacker")
    ...
    print("\nWhat happened:")
    print(f"  1. ✓ Generated {N_ATTACKER_PROMPTS} attacker prompts")
    print(f"  2. ✓ Tested on {M_QUERIES_PER_CATEGORY} HarmBench queries")
    print(f"  3. ✓ Created {N_ATTACKER_PROMPTS}x{M_QUERIES_PER_CATEGORY} evaluation matrix")
    print(f"  4. ✓ Ranked prompts for each query separately")
    print(f"  5. ✓ Aggregated rankings using Borda count")
    print(f"  6. ✓ Trained model using aggregated rankings")
...
def run_multi_category():
    """
    Run on multiple categories from HarmBench.
    """
    
    print("\n" + "="*80)
    print("MULTI-CATEGORY PIPELINE")
    print("="*80)
    ...
    rl_trainer = RankingToRLTrainer(MODEL_NAME)
    rl_trainer.train_from_rankings(
        category_rankings=all_category_rankings,
        method="reward_weighted",
        epochs=2,
        batch_size=2,
        save_path="./phi35_multi_category"
    )
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("PHI-3.5 RL PIPELINE WITH HARMBENCH")
    print("="*80)
    print("\nFeatures:")
    print("  • Uses Phi-3.5-mini for all components")
